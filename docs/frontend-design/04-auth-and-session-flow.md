...
- Зависит от: `src/shared/admin-auth.ts`, `src/admin/admin-app.tsx`, `src/shared/use-showcase-edit-permission.ts`, `src/admin/admin-settings-accounts-section.tsx`.
...
- logout-кнопки спящего `SiteLayout`
...
Этот hook импортируется в:
- исходники public-site
- shared/admin-adjacent UI
...
Защищено только routed admin-приложение. Текущая заглушка `SiteApp` не имеет auth-gate. Спящие site-страницы все еще могут вызывать auth-aware helpers, но эти страницы не смонтированы в текущем активном runtime.
