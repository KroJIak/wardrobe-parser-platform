...
- средняя уверенность только в исчерпывающем artifact-by-artifact enumeration, потому что docs намеренно суммируют большие наборы lab fixtures и developer-side assets вместо индексации каждого отдельного файла
