...
## Назначение
`parser-service` — это выделенная FastAPI-среда выполнения для парсинга storefront-источников и оркестрации sync-процессов. Он владеет файловой конфигурацией выполнения парсеров и состоянием sync-job/event в памяти процесса; основной путь записи в базу каталога ему не принадлежит.
...
- Долговечность этой конфигурации зависит от infra-mount: локально она подтверждена через `docker-compose.override.yml`, а без mount сервис использует копию файла из container filesystem.
