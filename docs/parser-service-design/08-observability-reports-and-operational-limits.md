...
## Граница относительно сохраняемых артефактов
В текущем inspected runtime-коде parser-service не публикует встроенную repo-backed persistence surface для отчетов или диагностических артефактов.
Операционная видимость строится через API-ответы, event stream и логи контейнера, а не через сохранение файловых отчетов в репозитории.
